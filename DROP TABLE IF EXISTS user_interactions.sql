...
SELECT game_id,
case when count(interaction_type) = 0 then 'No Interactions' 
     when count(distinct case when interaction_type is not null then user_id end ) = 1 then 'Single Sided Interaction'
     when count(distinct case when interaction_type is not null then user_id end ) = 2 and
      count( distinct case when interaction_type = 'custom_typed' then user_id end ) = 0  then 'Double Sided Interaction without custom typed message'
      when count(distinct case when interaction_type is not null then user_id end ) = 2 and
      count( distinct case when interaction_type = 'custom_typed' then user_id end ) >= 1  then 'Double Sided Interaction with custom typed message'
end as Type
